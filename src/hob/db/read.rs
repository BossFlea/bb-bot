use anyhow::Result;
use rusqlite::{Connection, Error, OptionalExtension as _, Statement, ToSql, params};
...
pub fn get_all_hob_entries(conn: &Connection) -> Result<Vec<HobEntry>, Error> {
...
pub fn get_hob_entry_by_id(conn: &Connection, id: u64) -> Result<Option<HobEntry>, Error> {
    let oneoff_statement = conn.prepare(
        "
        SELECT e.id, e.title, e.comment, e.bingo, e.bingo_kind, m.bingo AS sort_value
        FROM hob_entries_oneoff e
    ...
    query_entries(conn, oneoff_statement, ongoing_statement, params![id])
        .map(|vec| vec.into_iter().next())
}

fn get_oneoff_players(conn: &Connection, entry_id: u64) -> Result<Vec<String>, Error> {
...
fn get_ongoing_subentries(conn: &Connection, entry_id: u64) -> Result<Vec<OngoingSubentry>, Error> {
...
pub fn get_ongoing_subentry_by_id(
    conn: &Connection,
    id: u64,
    entry_id: u64,
) -> Result<Option<OngoingSubentry>, Error> {
    let mut statement = conn.prepare(
        "
...
pub fn search_entries_with_content(conn: &Connection, query: &str) -> Result<Vec<HobEntry>, Error> {
    let oneoff_statement = conn.prepare(
        "
    ...
    query_entries(conn, oneoff_statement, ongoing_statement, params![query])
}
